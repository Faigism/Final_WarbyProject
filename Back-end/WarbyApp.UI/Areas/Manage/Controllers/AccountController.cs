...
namespace WarbyApp.UI.Areas.Manage.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public async Task<IActionResult> Login(Login_VM login, string returnUrl)
        {
            ...
            using (var response = await _client.PostAsync("https://localhost:7125/api/accounts/login", content))
            {
                if (response.IsSuccessStatusCode)
                {
                    ...
                    HttpContext.Response.Cookies.Append("account_token", token);
